...
def on_performance_drop(model_evaluation_task_id, ml_pipeline, threshold=0.5):
    model_evaluation_task = Task.get_task(task_id=model_evaluation_task_id)
    test_accuracy = get_test_accuracy(model_evaluation_task.get_reported_plots())
    if test_accuracy < threshold: # Trigger if accuracy < 60%
        print(f'Validation Accuracy {test_accuracy} below threshold {threshold} => Triggering retraining.')
        ml_pipeline.start_locally() # Start the pipeline inside the same machine as the scheduler
...
# Add the actual trigger and enqueue the pipeline in services because we're cloning the
...
trigger.add_dataset_trigger( # Add trigger on dataset creation
    ...
    schedule_queue='services', # Cannot use the same queue with pipeline's tasks: https://github.com/clearml/clearml/issues/1328
...
trigger.start()
# trigger.start_remotely(queue='services') # Start the trigger remotely, so an agent will keep track of everything