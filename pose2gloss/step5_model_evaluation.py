...
model.summary(print_fn=lambda x: task.logger.report_text(x))
...
task.logger.report_table(table_plot=metrics_table, title='Model evaluation metrics on 3 subsets', series='ignored')

y_test_preds = np.argmax(model.predict(test_tf_dataset), axis=1)
classification_report_df = pd.DataFrame(classification_report(y_test, y_test_preds, output_dict=True, target_names=label_encoder.classes_)).T
task.logger.report_table(table_plot=classification_report_df, title='Classification report on test set', series='ignored')

cm = confusion_matrix(y_test, y_test_preds)
task.logger.report_confusion_matrix(
    matrix=cm, title='Confusion matrix', series='ignored',
    xlabels=label_encoder.classes_, ylabels=label_encoder.classes_,
    xaxis='Predicted gloss', yaxis='True gloss',
)