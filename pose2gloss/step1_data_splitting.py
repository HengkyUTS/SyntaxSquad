...
plt.tight_layout()
task.logger.report_matplotlib_figure(figure=plt, title=f"Distribution of Top {top_n}/{args['max_labels']} Glosses")

wordcloud = WordCloud(width=1000, height=500, background_color='white').generate_from_frequencies(glosses_counts)
plt.figure(figsize=(10, 5))
plt.imshow(wordcloud, interpolation='bilinear')
plt.axis('off')
task.logger.report_matplotlib_figure(figure=plt, title=f"Word Cloud of Top {top_n}/{args['max_labels']} Glosses")
...
task.logger.report_table(table_plot=final_stats, title=f"Dataset Statistics for Top {args['max_labels']} Glosses")
...
task.logger.report_plotly(figure=fig, title=f"Histogram of Video Length by Subset (Top {args['max_labels']} Glosses)")
...
task.logger.report_plotly(figure=fig, title=f"Boxplot of Video Length by Subset (Top {args['max_labels']} Glosses)")